...
namespace pyro.Scripts.Utils
{
    ...
    public class Database : IDatabase
    {
        ...
        public async Task<string> CreateUser(string username, string email, string password, long discordUserId)
        {
            ...
            if (await users.Find(p => p.discordUserId == discordUserId).FirstOrDefaultAsync() != null) // lovely
            {
                return "Már van regisztrált fiókod!";
            }

            if (await users.Find(p => p.username == username).FirstOrDefaultAsync() != null)
            {
                return "Ez a felhasználónév már foglalt!";
            }

            if (await users.Find(p => p.email == email).FirstOrDefaultAsync() != null)
            {
                return "Ezzel az e-mail-címmel már regisztrált valaki!";
            }

            string emailFilter = @"^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,7}$";

            if (!Regex.IsMatch(email, emailFilter))
            {
                return "Helytelen a megadott e-mail formátum!";
            }
